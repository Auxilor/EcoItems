...
The `item:` section is exactly the same as any other EcoItem. See [How to Make an Item](how-to-make-an-item) for the full breakdown of the base item, display, and recipe fields. A tool is still a full EcoItem, so the top-level `effects:` and `conditions:` work too, e.g. running an effect on a `mine_block` trigger alongside the mining rules.
...
:::info Existing tools update automatically
With `auto-update-items` on in `config.yml` (the default), tools in player inventories and ender chests are rebuilt from their current config on join and after a reload, and dropped items update when picked up. With it off, only newly given or crafted copies pick up changes.
:::
...
- **Custom foods:** [Custom Foods](item-types-foods) for eating behaviour instead of mining.
- **Repairing:** [Repairable Items](item-types-repairing) to let players repair tools at an anvil.

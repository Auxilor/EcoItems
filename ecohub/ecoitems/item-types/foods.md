...
3. Set up the `item:` section as you would for any item (see [How to Make an Item](how-to-make-an-item)).
...
The `item:` section is exactly the same as any other EcoItem. See [How to Make an Item](how-to-make-an-item) for the full breakdown of the base item, display, and recipe fields. A food is still a full EcoItem, so the top-level `effects:` and `conditions:` work too; a `consume` trigger pairs naturally with foods:
...
:::info Existing foods update automatically
With `auto-update-items` on in `config.yml` (the default), foods in player inventories and ender chests are rebuilt from their current config on join and after a reload, and dropped items update when picked up. With it off, only newly given or crafted copies pick up changes.
:::
...
- **Hunger doesn't change after editing?** Check `auto-update-items` is on in [Plugin Config](plugin-config), or get a fresh copy.
:::
...
- **Custom tools:** [Custom Tools](item-types-tools) for mining behaviour instead of eating.

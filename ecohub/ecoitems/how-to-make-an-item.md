...
The old `base-damage`, `base-attack-speed`, and `base-attack-range` options have been replaced by `minecraft:attribute_modifiers` (as above). They still work (they're converted into the matching modifiers, with a deprecation warning in the console), but the component is the format to write new items in. `effective-durability` has been replaced by `max_damage:<amount>` on the base item lookup. The old `food:` and `tool:` sections are now the `minecraft:food`, `minecraft:consumable`, and `minecraft:tool` components; see [Custom Foods](item-types-foods) and [Custom Tools](item-types-tools).
:::
...
On 26.3 and newer, the `minecraft:cooking_fuel` and `minecraft:brewing_fuel` components make any item a fuel; see [Custom Fuels](item-types-fuels). On older versions, items based on a vanilla fuel can override how long they burn:
...
Furnaces on older versions only consume items vanilla recognises as fuel, so base the item on one (`coal`, `blaze_rod`, ...) and tune from there.
...
- **Item types:** [Custom Foods](item-types-foods), [Custom Tools](item-types-tools), [Custom Weapons](item-types-weapons), [Custom Shields](item-types-shields), and more add behaviour to an item with components.
- **Durability:** [Repairable Items](item-types-repairing) to make an item wear out and repair at an anvil.
- **Workstations:** [Workstation Recipes](workstation-recipes) to craft the item at a furnace, anvil, stonecutter, and more.
- **Rarities:** [Item Rarity](item-rarity) for the rarity tags used by `rarity:`.

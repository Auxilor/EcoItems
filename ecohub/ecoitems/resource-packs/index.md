...
The paid version ships example content with its textures and models in the `pack/` folder: textured tools, furniture, mob armor, and paintings in `items/examples/`, plus the Blocks Expansion and Donuts sets in `items/defaults/`. Give one a try with `/ecoitems give <you> amethyst_pickaxe`. Delete the item configs if you don't want them; deleting the whole `pack/` folder restores the default assets on the next reload.
...
References are `[namespace:]path`, with no file extension, the path relative to `textures/`, and the namespace defaulting to `ecoitems`. Keep item textures under `textures/item/` (or `textures/block/`) so the block atlas stitches them; a console warning fires otherwise. EcoItems generates a simple model for the texture unless you put your own model JSON at the same path under `models/` (e.g. `pack/assets/ecoitems/models/item/mithril_sword.json`).
...
| Path | Purpose |
| --- | --- |
| `pack/assets/ecoitems/textures/` | Textures - items under `textures/item/`, [glyphs](how-to-make-a-glyph) under `textures/glyph/` |
| `pack/assets/ecoitems/models/` | Custom item models (`.json`), referenced by `model:` |
| `pack/assets/ecoitems/sounds/` | [Custom sound](how-to-make-a-sound) files (`.ogg`) |

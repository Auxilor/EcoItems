...
| `/ecoitems hud toggle <id>`               | Toggle a [HUD](huds) on or off                                     | `ecoitems.command.hud.toggle` (everyone) |
| `/ecoitems glyph <id>`                    | Echo a [glyph](glyphs)'s character, e.g. to copy it from console   | `ecoitems.command.glyph` (operators)     |
| `/ecoitems glyphs`                        | Open the [glyph picker](glyphs) book (click a glyph to chat it)   | `ecoitems.command.glyphs` (everyone)     |
...
| `/ecoitems dialog <id> [player]`          | Open a [dialog](dialogs) screen                                    | `ecoitems.command.dialog` (operators)    |
...
[HUDs](huds) with a `permission:` in their config additionally require that permission to be seen at all.
...
[Glyphs](glyphs) with a `permission:` in their config (like the shipped chat tags, `ecoitems.glyph.<id>`) require
